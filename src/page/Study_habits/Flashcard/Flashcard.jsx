...
const Flashcards = ({ subject = "Computer Programming" }) => {
    const hardcodedFlashcards = [
        ...
        { question: "What is a prop?", answer: "A way to pass data from parent to child components." },
        { question: "What is useEffect?", answer: "A React Hook that runs side effects in functional components." }
    ];
    ...
    const displayedCards = hardcodedFlashcards.slice(startIndex, startIndex + cardsPerPage);
            ...
            <div className={styles.cardContainer}>
                {displayedCards.map((flashcard, index) => {
                    ...
                    return (
                        <div
                            key={globalIndex}
                            className={`${styles.card} ${flippedCards[globalIndex] ? styles.flipped : ""}`}
                            onClick={() => handleFlip(globalIndex)}
                        >
                            <div className={styles.inner}>
                                <div className={`${styles.front} ${flippedCards[globalIndex] ? styles.hidden : ""}`}>
                                    <p>{flashcard.question}</p>
                                </div>
                                <div className={`${styles.back} ${flippedCards[globalIndex] ? styles.visible : styles.hidden}`}>
